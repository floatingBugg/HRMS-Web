...
@Component({
  selector: 'app-power-cables',
  templateUrl: './power-cables.component.html',
  styleUrls: ['./power-cables.component.scss']
})
export class PowerCablesComponent implements OnInit {
    powerCableForm: any = FormGroup;
    errorMsg: any;
  constructor(
  ...
  createForm() {
    this.powerCableForm = this.fb.group({
      itacCategoryId: 9,
  ...
  submitData() {
    ...
    this.inventory
      .postAssets(this.powerCableForm.value)
      .subscribe((result) => {
        if (result.success) {
          Swal.fire({
            title: 'Added!',
            ...
            this.router.navigate(["inventory/unassigned-power-cables"])
          })
        } 
        else 
        {
